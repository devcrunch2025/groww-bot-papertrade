...
app.listen(PORT, async () => {
  ...
  const { startAllEngines } = require("./strategyEngine");
  await startAllEngines(10000, async ({ snapshot, strategyId }) => {
    await sendNotificationsForLatestCycle(snapshot, `auto-cycle-${strategyId}`);
  });
  setInterval(() => {
